...
{	
	...
	{	
        ...
        //A boolean to tell whether or not the to use the idle animation
		...
		{
            // if we're running, not grounded, and have pressed the jump button, we change our state to Jumping
            if ((_inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonDown || _inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed)
            ...
            else if (!_controller.State.IsGrounded && (_movement.CurrentState == CharacterStates.MovementStates.Running))
			{
				_movement.ChangeState(CharacterStates.MovementStates.Falling);
			}
			// if we're not moving fast enough, we go back to idle
			if ((Mathf.Abs(_controller.Speed.x) <= _characterBasicMovement.WalkSpeed) && (_movement.CurrentState == CharacterStates.MovementStates.Running))
			{
                //Switch the shouldIdle boolean but don't change the character state, as the character could still have running "active"
                shouldIdle = true;
			}
		}
		...
		{
            //If the character has running active, but is standing still
            if(shouldIdle)
            {
                //Switch to the idle animation
                MMAnimator.UpdateAnimatorBool(_animator, "Idle", true, _character._animatorParameters);
                MMAnimator.UpdateAnimatorBool(_animator, "Running", false, _character._animatorParameters);
            }
            else
            {
