...
{	
	...
	{	
        ...
        [Header("Multi-Jump Sound")]
        /// the sound to play when multi-jumping
        public AudioClip MultiJumpSfx;
		...
		{
            if (_movement.CurrentState == CharacterStates.MovementStates.Falling && _inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
			{
                NumberOfJumpsLeft -= 1;
				JumpStart();
			}
            else if (_inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
            {
                JumpStart();
            }
		...
		{			
            ...
            if (NumberOfJumpsLeft != NumberOfJumps || _movement.CurrentState == CharacterStates.MovementStates.Falling)
            {
                PlayDoubleJumpStartSfx();
                _doubleJumping = true;
            }
            else
            {
        ...
        protected virtual void PlayDoubleJumpStartSfx()
        {
            if (MultiJumpSfx != null)
            {
                SoundManager.Instance.PlaySound(MultiJumpSfx, transform.position);
            }
        }
    }
}
