...
namespace MoreMountains.CorgiEngine
{
    ...
    public class CharacterHorizontalMovement : CharacterAbility
    {
        ...
        [Header("Additional Movement Sound")]
        /// the sound to play ONLY when the character is moving
        public AudioClip MovementInProgressSfx;
        ...
        protected AudioSource _movementInProrgressSfx;

        private CharacterStates.MovementStates[] wheelStates = {CharacterStates.MovementStates.Walking, CharacterStates.MovementStates.Running};
        ...
        /// <summary>
        /// Distort the pitch of all audio sources upwards
        /// </summary>
        public virtual void pitchUp()
        {
            base.pitchShift(1.1f);

            if(_movementInProrgressSfx != null)
            {
                _movementInProrgressSfx.pitch = 1.1f;
            }
        }
        ...
        {
            // if running is engaged, but the character is in a different state
            if ((_movement.CurrentState == CharacterStates.MovementStates.Falling 
                ...
                && _inputManager != null && (_inputManager.RunButton.State.CurrentState == MMInput.ButtonStates.ButtonDown
                || _inputManager.RunButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed))
            {
                //Do not stop the running sfx
            }
            else if (_movement.CurrentState == CharacterStates.MovementStates.Running)
			{
				//Do not stop running sfx
			}
            else
            {
                PlayAbilityUsedSfx();
                base.pitchShift(1.0f);
            }

            //If the character is walking or running, play an additional movement sfx
            characterRun = GetComponent<CharacterRun>();
            if(characterRun != null)
            {
                if (wheelStates.Contains(_movement.CurrentState) && !(Mathf.Abs(_controller.Speed.x) <= characterRun.RunSpeed / 10))
                {
                    PlayMovementInProgressSfx();
                }
                else
                {
                    StopMovementInProgressSfx();
                }
            }
			...
			// if we're grounded and moving, and currently Idle or Dangling, we become Walking
        ...
        /// <summary>
        /// Begin looping the additional movement sfx if provided
        /// </summary>
        protected virtual void PlayMovementInProgressSfx()
        {
            if (MovementInProgressSfx != null)
            {
                if (_movementInProrgressSfx == null)
                {
                    _movementInProrgressSfx = SoundManager.Instance.PlaySound(MovementInProgressSfx, transform.position, true);
                    _movementInProrgressSfx.pitch = 1.0f;
                }
            }
        }

        /// <summary>
        /// Stop looping the additional movement sfx
        /// </summary>
        protected virtual void StopMovementInProgressSfx()
        {
            if (MovementInProgressSfx != null)
            {
                SoundManager.Instance.StopLoopingSound(_movementInProrgressSfx);
                _movementInProrgressSfx = null;
            }
        }
